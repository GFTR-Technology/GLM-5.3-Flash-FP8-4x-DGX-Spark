...
This FP8 card is ~78 GiB of weights **per rank**. Tensor parallel 4 is how it fits. See [2-Spark notes](#two-sparks) at the bottom if you only have two boxes, or [Six Sparks](#six-sparks) if you have six — TP=6 divides none of this model's head counts and needs the padding shim.
...
TP comes from the node count, so six IPs in `cluster.env` means TP=6 — and TP=6
divides none of this model's head counts, its MoE intermediate, or its vocab.
The launcher notices and wires in the padding shim automatically; you do not set
anything extra.

```bash
WORKER_IPS="10.0.0.12 10.0.0.13 10.0.0.14 10.0.0.15 10.0.0.16"

./scripts/glm53-tp-probe.sh            # once, before the first boot
...
| `divide()` / `assert ... % tp_size` at start-up | A TP that does not divide the model. Six nodes: see [Six Sparks](#six-sparks) and [docs/TP6.md](TP6.md). |
...
| `divide(16, 6)` in the vision tower, or a vLLM warning about falling back from `--mm-encoder-tp-mode data` | The `supports_encoder_tp_data` marker did not reach the process that resolves the config. vLLM decides this in a registry-inspection subprocess, so check `PYTHONPATH=/opt/glm53` and `GLM53_TP_PAD` are on the container (not just the ranks). Ranks log `[glm53-tp-pad] supports_encoder_tp_data=True on Glm5NextForConditionalGeneration`; the subprocess does not, its output is discarded. If the model module moved, add its path to `HOOKS` in `glm53_tp_pad.py`. |
