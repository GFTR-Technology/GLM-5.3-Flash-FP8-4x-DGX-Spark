...
# A TP that does not divide 64 heads / 32 indexer heads / the 2048 MoE
# intermediate / the 154880 vocab needs the padding shim bind-mounted on every
# rank, not just the head. Catch a half-deployed clone here rather than 12
# minutes into a load. See docs/TP6.md.
TP="$NNODES"
if [ $((64 % TP)) -ne 0 ] || [ $((32 % TP)) -ne 0 ] \
  ...
  for ip in "${NODES[@]}"; do
    if ! ssh_to "$ip" "test -f $PATCH_HOST/glm53_tp_pad/glm53_tp_pad.py && test -f $PATCH_HOST/glm53_tp_pad/sitecustomize.py && test -f $PATCH_HOST/glm53_tp_pad/make_overlay.py"; then
      echo "Error: $PATCH_HOST/glm53_tp_pad/ incomplete on $ip"
      echo "       Pull this repo on every node, then re-run."
      exit 1
    fi
  done
  echo "       run ./scripts/glm53-tp-probe.sh first if this image is new to you"
fi
